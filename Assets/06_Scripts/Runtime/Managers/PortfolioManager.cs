...
namespace RFB.Portfolio
{
    public class PortfolioManager : Singleton<PortfolioManager>
    {
        ...
        // Icon download ID
        public string iconProgressID = "PORTFOLIO";
        public string galleryProgressID = "GALLERY";
        ...
        private void LoadIcon(int projectIndex)
        {
            ...
            }, delegate(float progress)
            {
                ...
                AppManager.instance.SetLoadProgress(iconProgressID, p);
            });
        }
        // Load complete
...
#region SELECTION
        ...
        public void LoadImage(string iconName, Action<Texture2D> onComplete, Action<float> onProgress = null)
        {
            ...
            }, true, delegate(float p)
            {
                AppManager.instance.SetLoadProgress(galleryProgressID, p);
                if (onProgress != null)
                {
                    onProgress(p);
                }
